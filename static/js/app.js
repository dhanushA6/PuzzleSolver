...
const redo = document.querySelector('.do-btn');
...
let solved_toggle = true;
// --------

const getGameInfo = () => JSON.parse(localStorage.getItem('game'));

// add space for each 9 cells
// code to make 3x3 grid 
const initGameGrid = () => {
    let index = 0;
...
const init = () => {
    ...
    document.querySelector('.solution-btn').style.backgroundColor = solved_toggle ? 'red': 'green'
    ...
    cells.forEach((e, index) => {
        e.addEventListener('click', async () => {
            if (!e.classList.contains('filled')) {
                ...
                let game = getGameInfo();
                if (solved_sud !=  -1) {
                    su_answer[row][col] = solved_sud[row][col]
                    // save game
                    saveGameInfo()
                    // -----
                    removeErr();
                    cells[selected_cell].innerHTML = solved_sud[row][col];
                    cells[selected_cell].setAttribute('data-value', solved_sud[row][col]);
                    // cells[selected_cell].classList.add('filled')
                    // check game win'
...
document.querySelector('.solution-btn').addEventListener('click', async () => {
    // Send Sudoku to Flask for solving
    const do_solve = async () => {
        await fetch('/solve_sudoku', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                },
                body: JSON.stringify({ sudoku: su_answer }),
            })
            .then(response => response.json())
            .then(data => {
                let solvedSudoku = data.solved_sudoku;
                console.log("Solved Sudoku:", solvedSudoku);
                if (solved_toggle) {
                       solved_sud = [...solvedSudoku];
                       console.log(solved_sud)
                       document.querySelector('.solution-btn').style.backgroundColor = 'green'
                       solved_toggle = false
                }
                else  {
                    solved_sud =  -1;
                    document.querySelector('.solution-btn').style.backgroundColor = 'red'
                    solved_toggle = true
                }
